...
const candidateQaSection = (c) => `
  <section class="section qa-section">
    <div class="section-head"><h2>Ask about ${esc(c.name)}</h2><span class="count">AI · Groq · U.S. elections only</span></div>
    <p class="qa-note">Answers are generated by an AI model grounded in this candidate's Pollbook profile (FEC filings, Wikipedia, news) — it will only discuss United States elections and candidates, and refuses anything else. It can get things wrong; verify anything that matters against the sources on this page. Your conversation is stored only in this browser, never on our server.</p>
    ...
    <form id="qa-form" class="qa-form">
      ...
      <button type="submit" class="filter-btn">Ask</button>
    </form>
